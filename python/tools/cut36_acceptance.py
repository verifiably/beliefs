"""Run cut 36 after cut 35 on the certified durable tuple."""
...
DEFAULT_WORK = MAIN_CHECKOUT / ".work" / "acceptance" / "cut36"

PREFIX_RUNNERS = ("cut35_acceptance.py",)
...
def declared_accounting() -> tuple[int, int, int]:
    ...
    from n2_arms_cut36 import CUT36_ARMS, DECLARATION_UNITS  # pyright: ignore[reportMissingImports]

    rows = {unit.partition("-")[0] for unit in DECLARATION_UNITS if not unit.startswith("BI-")}
    assert rows == {"L8", "L4", "L10"}
    return len(CUT36_ARMS), len(DECLARATION_UNITS), len(rows)
