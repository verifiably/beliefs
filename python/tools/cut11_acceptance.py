...
the unedited cut-10 prefix, durable cut-11 arms, then cut-11's N2 audit.
...
DEFAULT_WORK = MAIN_CHECKOUT / ".cut11-acceptance"
...
def cut_environment(run: Path) -> dict[str, str]:
    return {
        **os.environ,
        **{f"SCIENCE_CUT{number}_ROOT": str(run) for number in range(4, 12)},
    }
...
def main(argv: list[str]) -> int:
    ...
    for module in PHASE_MODULES:
        ...
        if not path.is_file():
            print(f"cut-11 required acceptance module is missing: {path}", file=sys.stderr)
            return 1
    for runner in PREFIX_RUNNERS:
        path = TOOLS / runner
        if not path.is_file():
            print(f"cut-11 required prefix runner is missing: {path}", file=sys.stderr)
            return 1
    ...
    try:
        ...
        try:
            ...
            print(
                f"cut-11 acceptance: could not compute declared-arm count: {failure}",
                file=sys.stderr,
            )
        else:
            print(
