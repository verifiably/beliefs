"""Run cut 22 after cut 21 on the certified durable tuple."""
...
DEFAULT_WORK = PYTHON_ROOT.parent / ".cut22-acceptance"

PREFIX_RUNNERS = ("cut21_acceptance.py",)
PHASE_MODULES = ("test_biology_acceptance.py", "test_n2_cut22.py")
PROBE_REFUSED = 2


def work_directory() -> Path:
    work = Path(os.environ.get("SCIENCE_CUT22_ROOT", DEFAULT_WORK))
...
def probe(run: Path) -> str | None:
    ...
    try:
        init_corpus_root(corpus_root, authority=Authority(WritePermit.full(), "cut22-probe"))
        return None
...
def declared_accounting() -> tuple[int, int, int]:
    ...
    from n2_arms_cut22 import CUT22_ARMS, DECLARATION_UNITS, unit_of  # pyright: ignore[reportMissingImports]

    return len(CUT22_ARMS), len(DECLARATION_UNITS), len({unit_of(arm.row) for arm in CUT22_ARMS})
...
def cut_environment(run: Path) -> dict[str, str]:
    return {
        ...
        **{f"SCIENCE_CUT{number}_ROOT": str(run) for number in range(4, 23)},
    }
...
def main(argv: list[str]) -> int:
    ...
    for module in PHASE_MODULES:
        if not (ACCEPTANCE / module).is_file():
            print(f"cut-22 required acceptance module is missing: {ACCEPTANCE / module}", file=sys.stderr)
            return 1
    for runner in PREFIX_RUNNERS:
        if not (TOOLS / runner).is_file():
            print(f"cut-22 required prefix runner is missing: {TOOLS / runner}", file=sys.stderr)
            return 1
    ...
    try:
        ...
        if refusal is not None:
            print(
                "cut-22 acceptance cannot run here: its durable corpus prerequisite refused.\n"
                f"  {refusal}\n"
                "  Set SCIENCE_CUT22_ROOT to a certified volume or recertify the tuple. "
        ...
        for runner in PREFIX_RUNNERS:
            phase += 1
            print(f"[cut22 phase {phase}/{phases}] {runner}", flush=True)
        ...
        for index, module in enumerate(PHASE_MODULES):
            phase += 1
            print(f"[cut22 phase {phase}/{phases}] {module}", flush=True)
            completed = subprocess.run(
                [
                    sys.executable,
                    "-m",
