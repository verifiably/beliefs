"""Run cut 20 after cut 19 on the certified durable tuple."""
...
def work_directory() -> Path:
    work = Path(os.environ.get("SCIENCE_CUT20_ROOT", DEFAULT_WORK))
...
def probe(run: Path) -> str | None:
    ...
    try:
        init_corpus_root(corpus_root, authority=Authority(WritePermit.full(), "cut20-probe"))
        return None
...
def declared_accounting() -> tuple[int, int]:
    ...
    from n2_arms_cut20 import CUT20_ARMS, DECLARATION_UNITS  # pyright: ignore[reportMissingImports]

    return len(CUT20_ARMS), len(DECLARATION_UNITS)
...
def cut_environment(run: Path) -> dict[str, str]:
    return {
        ...
        **{f"SCIENCE_CUT{number}_ROOT": str(run) for number in range(4, 21)},
    }
...
def main(argv: list[str]) -> int:
    ...
    for module in PHASE_MODULES:
        if not (ACCEPTANCE / module).is_file():
            print(f"cut-20 required acceptance module is missing: {ACCEPTANCE / module}", file=sys.stderr)
            return 1
    for runner in PREFIX_RUNNERS:
        if not (TOOLS / runner).is_file():
            print(f"cut-20 required prefix runner is missing: {TOOLS / runner}", file=sys.stderr)
            return 1
    ...
    try:
        ...
        if refusal is not None:
            print(
                "cut-20 acceptance cannot run here: its durable corpus prerequisite refused.\n"
                f"  {refusal}\n"
                "  Set SCIENCE_CUT20_ROOT to a certified volume or recertify the tuple. "
        ...
        for runner in PREFIX_RUNNERS:
            phase += 1
            print(f"[cut20 phase {phase}/{phases}] {runner}", flush=True)
        ...
        for index, module in enumerate(PHASE_MODULES):
            phase += 1
            print(f"[cut20 phase {phase}/{phases}] {module}", flush=True)
            completed = subprocess.run(
                [
                    sys.executable,
                    "-m",
        ...
        print(
            f"declared arms: {arms} (= {units} declaration units; 16 guarantee rows + parity fixture + boundary invariant)",
            flush=True,
        )
        print(
