"""Run cut 27 after cut 26 on the certified durable tuple."""
...
def declared_accounting() -> tuple[int, int, int]:
    ...
    from n2_arms_cut27 import CUT27_ARMS, DECLARATION_UNITS, unit_of  # pyright: ignore[reportMissingImports]

    return len(CUT27_ARMS), len(DECLARATION_UNITS), len({unit_of(arm.row) for arm in CUT27_ARMS})
