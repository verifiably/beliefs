"""Run cut 30 after cut 29 on the certified durable tuple."""
...
DEFAULT_WORK = PYTHON_ROOT.parent / ".cut30-acceptance"

PREFIX_RUNNERS = ("cut29_acceptance.py",)
PHASE_MODULES = ("test_source_address_acceptance.py", "test_n2_cut30.py")
...
def declared_accounting() -> tuple[int, int, int]:
    ...
    from n2_arms_cut30 import CUT30_ARMS, DECLARATION_UNITS, unit_of  # pyright: ignore[reportMissingImports]

    return len(CUT30_ARMS), len(DECLARATION_UNITS), len({unit_of(arm.row) for arm in CUT30_ARMS})
