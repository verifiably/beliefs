"""Run cut 28 after cut 27 on the certified durable tuple."""
...
DEFAULT_WORK = PYTHON_ROOT.parent / ".cut28-acceptance"

PREFIX_RUNNERS = ("cut27_acceptance.py",)
PHASE_MODULES = ("test_world_selection_acceptance.py", "test_n2_cut28.py")
...
def declared_accounting() -> tuple[int, int, int]:
    ...
    from n2_arms_cut28 import CUT28_ARMS, DECLARATION_UNITS, unit_of  # pyright: ignore[reportMissingImports]

    return len(CUT28_ARMS), len(DECLARATION_UNITS), len({unit_of(arm.row) for arm in CUT28_ARMS})
