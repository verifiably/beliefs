"""Cut 31 declaration accounting, freeze pin, and N2 audit."""
...
from n2_arms_cut31 import CO_CITED, CUT31_ARMS, DECLARATION_UNITS, UNIT_CHECKS, unit_of
...
FROZEN_CUT = REPO_ROOT / "docs" / "designs" / "2026-09-15-conformance-cut-31.md"
CUT31_FREEZE_COMMIT = "c2a2211c2d9a0889a59e7892dcb71f2008e20e46"
CUT31_FROZEN_SHA256 = "3cd4409dd08d5b121d3f62bfaaa00e3d553335d6a54e7657471c70677854d93f"
FROZEN_DECLARATION = "python/tests/n2_arms_cut31.py"
CUT31_DECLARATION_SHA256 = "50a32687a11a3ea3aba01c288c10d1b15ce98010f03cd0c7d7313a67c5c30a3b"
...
def findings(tmp_path_factory):
    workspace = tmp_path_factory.mktemp("n2-cut31")
    with ThreadPoolExecutor(max_workers=WORKERS) as pool:
        return tuple(
            pool.map(
                lambda pair: audit(pair[1], workspace / f"arm{pair[0]}"),
                enumerate(CUT31_ARMS),
            )
        )


def test_the_inventory_is_exactly_the_ten_declared_units() -> None:
    assert DECLARATION_UNITS == ("Q1", "Q2", "Q3", "Q4", "Q5", "Q6", "Q7", "Q8", "Q9", "Q10")
    assert {unit_of(arm.row) for arm in CUT31_ARMS} == set(DECLARATION_UNITS)
    assert len(CUT31_ARMS) == 26
    assert len(DECLARATION_UNITS) == 10
    assert all(arm.sabotage.package == "beliefs" for arm in CUT31_ARMS)
    assert set(UNIT_CHECKS) == set(DECLARATION_UNITS)
    assert set(UNIT_CHECKS.values()) <= {check for arm in CUT31_ARMS for check in arm.checks}
    # The cut document's §5 homes the arms by declaration unit; the table here
    # is that homing, counted.
    homed = {unit: sum(1 for arm in CUT31_ARMS if unit_of(arm.row) == unit) for unit in DECLARATION_UNITS}
    assert homed == {"Q1": 2, "Q2": 3, "Q3": 7, "Q4": 1, "Q5": 3, "Q6": 3, "Q7": 3, "Q8": 1, "Q9": 1, "Q10": 2}


def test_each_lettered_arm_is_unique_and_carries_an_exact_check() -> None:
    rows = [arm.row for arm in CUT31_ARMS]
...
def test_each_sabotage_names_one_real_source_site_and_keeps_the_module_importable() -> None:
    """Exactly one occurrence, and the mutated module still parses: a sabotage
    that turns a module into a syntax error is not a sabotage of the check it
    names — `pytest` exits on a usage error, which is not a failing test."""
...
def test_every_acceptance_test_the_arms_name_exists() -> None:
    module = REPO_ROOT / "python" / "tests" / "acceptance" / "test_estimand_acceptance.py"
    names = {
        node.name
...
def test_every_live_check_resolves_and_passes_without_sabotage() -> None:
    every = Arm(
        row="N2",
        asserts="every cut-31 check passes against the real package",
        sabotage=CUT31_ARMS[0].sabotage,
        checks=tuple(dict.fromkeys(check for arm in CUT31_ARMS for check in arm.checks)),
    )
...
def _frozen_body(text: str) -> str:
    ...
    The dated supplement in §8 is outside the original freeze, using the same
    slicing rule as cuts 19, 25 and 30.
    """
...
def test_the_freeze_commit_and_sections_two_through_seven_are_pinned() -> None:
    ...
    assert (
        subprocess.run(
            ["git", "-C", str(REPO_ROOT), "merge-base", "--is-ancestor", CUT31_FREEZE_COMMIT, "HEAD"],
            check=False,
        ).returncode
        == 0
    ), CUT31_FREEZE_COMMIT
    current = FROZEN_CUT.read_text(encoding="utf-8")
    frozen = _show(CUT31_FREEZE_COMMIT, str(FROZEN_CUT.relative_to(REPO_ROOT)))
    assert sha256(frozen.encode("utf-8")).hexdigest() == CUT31_FROZEN_SHA256
    ...
    assert '("cut30_acceptance.py",)' in current
...
def test_row_parser_accepts_only_declared_units_and_one_letter_suffix() -> None:
    ...
    for row in ("", "D1", "Q11", "Q1-", "Q1a", "Q1-A", "Q1-1", "Q1-aa", "Q1-a-b"):
        with pytest.raises(ValueError, match="is not a cut-31 row"):
            unit_of(row)
