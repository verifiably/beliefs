"""Cut 40 declaration accounting, freeze pin, and N2 audit."""
...
FROZEN_CUT = REPO_ROOT / "docs" / "designs" / "2026-09-24-conformance-cut-40.md"
CUT40_FREEZE_COMMIT = "34e553714d27f36d6131c4ea336b61a4f829cdcb"
CUT40_FROZEN_SHA256 = "4d3afb537df9a127e5d7ce6f473a2bbb098a4ee26a71f66b78955968a090f0ec"
FROZEN_DECLARATION = "python/tests/n2_arms_cut40.py"
CUT40_DECLARATION_SHA256 = "d24688e4642594b21aa7998163dc3d38245d30dc099c81dcbbad72f3e7c17d03"

# The frozen cut's §4: every Task 0 engine probe holds, so no arm is unrun.
FROZEN_ARMS, FROZEN_UNITS = 15, 15
...
def findings(tmp_path_factory):
    workspace = tmp_path_factory.mktemp("n2-cut40")
    with ThreadPoolExecutor(max_workers=WORKERS) as pool:
        return tuple(
            pool.map(
                lambda pair: audit(pair[1], workspace / f"arm{pair[0]}"),
                enumerate(CUT40_ARMS),
            )
        )
...
def test_the_inventory_is_exactly_the_frozen_rows_units() -> None:
    ...
    assert (FROZEN_ARMS, FROZEN_UNITS) == (15, 15)
    assert len(CUT40_ARMS) == FROZEN_ARMS and len(DECLARATION_UNITS) == FROZEN_UNITS
    assert all(arm.sabotage.package == "beliefs" for arm in CUT40_ARMS)
    assert set(UNIT_CHECKS) == set(DECLARATION_UNITS)
    assert {check for arm in CUT40_ARMS for check in arm.checks} == set(UNIT_CHECKS.values())
    homed = {unit: sum(unit_of(arm.row) == unit for arm in CUT40_ARMS) for unit in DECLARATION_UNITS}
    assert homed == dict.fromkeys(DECLARATION_UNITS, 1)


def test_each_arm_is_unique_and_carries_an_exact_check() -> None:
    rows = [arm.row for arm in CUT40_ARMS]
    assert len(rows) == len(set(rows))
    befores = [arm.sabotage.before for arm in CUT40_ARMS]
    assert len(befores) == len(set(befores))
    for arm in CUT40_ARMS:
...
def test_every_acceptance_test_the_arms_name_exists() -> None:
    ...
    module = REPO_ROOT / "python" / "tests" / "acceptance" / "test_publish_act_acceptance.py"
    names = {
        node.name
...
def test_every_live_check_resolves_and_passes_without_sabotage() -> None:
    every = Arm(
        row="N2",
        asserts="every cut-40 check passes against the real package",
        sabotage=CUT40_ARMS[0].sabotage,
        checks=tuple(dict.fromkeys(check for arm in CUT40_ARMS for check in arm.checks)),
    )
...
def test_the_freeze_commit_and_sections_two_through_seven_are_pinned() -> None:
    ...
    assert (
        subprocess.run(
            ["git", "-C", str(REPO_ROOT), "merge-base", "--is-ancestor", CUT40_FREEZE_COMMIT, "HEAD"],
            check=False,
        ).returncode
        == 0
    ), CUT40_FREEZE_COMMIT
    current = FROZEN_CUT.read_text(encoding="utf-8")
    frozen = _show(CUT40_FREEZE_COMMIT, str(FROZEN_CUT.relative_to(REPO_ROOT)))
    assert sha256(frozen.encode("utf-8")).hexdigest() == CUT40_FROZEN_SHA256
    ...
    assert "**15 arms, 15 declaration units**" in " ".join(current.split())
    assert '("cut39_acceptance.py",)' in current
...
def test_the_declaration_is_byte_exact_against_its_pinned_digest() -> None:
    ...
    assert sha256(current).hexdigest() == CUT40_DECLARATION_SHA256
    shim = (REPO_ROOT / "python" / "tests" / "acceptance" / "n2_arms_cut40.py").read_text(encoding="utf-8")
    assert "n2_arms_cut40.py" in shim  # the acceptance copy re-exports, never restates
...
def test_row_parser_accepts_only_exact_declared_units() -> None:
    ...
    for row in (
        ...
        with pytest.raises(ValueError, match="is not a cut-40 row"):
            unit_of(row)
