"""Cut 34 declaration accounting, freeze pin, and N2 audit."""
...
from n2_arms_cut34 import CO_CITED, CUT34_ARMS, DECLARATION_UNITS, UNIT_CHECKS, unit_of
...
FROZEN_CUT = REPO_ROOT / "docs" / "designs" / "2026-09-19-conformance-cut-34.md"
CUT34_FREEZE_COMMIT = "5eda3fa45b64bf716f44ae1de7adc5fd65a2b296"
CUT34_FROZEN_SHA256 = "97ba92153bd2866a09711c25d2fd3e1ee3ad0cc00591c41fa6f61dfb11588c3e"
FROZEN_DECLARATION = "python/tests/n2_arms_cut34.py"
CUT34_DECLARATION_SHA256 = "76e8e3b438a9468682d93900983407df4e6ee66120d99b6669ba7edd296ff1f8"
...
def findings(tmp_path_factory):
    workspace = tmp_path_factory.mktemp("n2-cut34")
    with ThreadPoolExecutor(max_workers=WORKERS) as pool:
        return tuple(
            pool.map(
                lambda pair: audit(pair[1], workspace / f"arm{pair[0]}"),
                enumerate(CUT34_ARMS),
            )
        )


def test_the_inventory_is_exactly_the_seventeen_declared_units() -> None:
    ...
    assert len(CUT34_ARMS) == len(DECLARATION_UNITS) == 17
    assert all(arm.sabotage.package == "beliefs" for arm in CUT34_ARMS)
    assert set(UNIT_CHECKS) == set(DECLARATION_UNITS)
    assert {check for arm in CUT34_ARMS for check in arm.checks} == set(UNIT_CHECKS.values())
    homed = {unit: sum(unit_of(arm.row) == unit for arm in CUT34_ARMS) for unit in DECLARATION_UNITS}
    assert homed == {unit: 1 for unit in DECLARATION_UNITS}


def test_each_arm_is_unique_and_carries_an_exact_check() -> None:
    rows = [arm.row for arm in CUT34_ARMS]
    assert len(rows) == len(set(rows))
    for arm in CUT34_ARMS:
...
def test_every_acceptance_test_the_arms_name_exists() -> None:
    module = REPO_ROOT / "python" / "tests" / "acceptance" / "test_snapshot_retraction_acceptance.py"
    names = {
        node.name
...
def test_every_live_check_resolves_and_passes_without_sabotage() -> None:
    every = Arm(
        row="N2",
        asserts="every cut-34 check passes against the real package",
        sabotage=CUT34_ARMS[0].sabotage,
        checks=tuple(dict.fromkeys(check for arm in CUT34_ARMS for check in arm.checks)),
    )
...
def test_the_freeze_commit_and_sections_two_through_seven_are_pinned() -> None:
    ...
    assert (
        subprocess.run(
            ["git", "-C", str(REPO_ROOT), "merge-base", "--is-ancestor", CUT34_FREEZE_COMMIT, "HEAD"],
            check=False,
        ).returncode
        == 0
    ), CUT34_FREEZE_COMMIT
    current = FROZEN_CUT.read_text(encoding="utf-8")
    frozen = _show(CUT34_FREEZE_COMMIT, str(FROZEN_CUT.relative_to(REPO_ROOT)))
    assert sha256(frozen.encode("utf-8")).hexdigest() == CUT34_FROZEN_SHA256
    assert _frozen_body(current) == _frozen_body(frozen)
    assert "**17 declaration units**" in current
    assert '("cut33_acceptance.py",)' in current
...
def test_the_declaration_is_byte_exact_against_its_pinned_digest() -> None:
    ...
    assert sha256(current).hexdigest() == CUT34_DECLARATION_SHA256
    shim = (REPO_ROOT / "python" / "tests" / "acceptance" / "n2_arms_cut34.py").read_text(encoding="utf-8")
    assert "n2_arms_cut34.py" in shim  # the acceptance copy re-exports, never restates
...
def test_row_parser_accepts_only_exact_declared_units() -> None:
    for unit in DECLARATION_UNITS:
        ...
        with pytest.raises(ValueError, match="is not a cut-34 row"):
            unit_of(f"{unit}-a")
    for row in ("", "C8", "C8-e", "C9", "BI", "BI-10", "BI-1-a", "C7-a", "U1"):
        with pytest.raises(ValueError, match="is not a cut-34 row"):
            unit_of(row)
