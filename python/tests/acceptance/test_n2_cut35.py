"""Cut 35 declaration accounting, freeze pin, and N2 audit."""
...
from n2_arms_cut35 import CO_CITED, CUT35_ARMS, DECLARATION_UNITS, UNIT_CHECKS, unit_of
...
FROZEN_CUT = REPO_ROOT / "docs" / "designs" / "2026-09-20-conformance-cut-35.md"
CUT35_FREEZE_COMMIT = "f746cf7cf42e4baaf537fd408d86925716ee9c2f"
CUT35_FROZEN_SHA256 = "d669f076f4dab0474888108d230a15e120cc76da4164a44446cf5d62ef91ddb0"
FROZEN_DECLARATION = "python/tests/n2_arms_cut35.py"
CUT35_DECLARATION_SHA256 = "5a384e510ebfdfe2ffad70bf6990f869a96e66c899e139959e053e35431e48df"
...
def findings(tmp_path_factory):
    workspace = tmp_path_factory.mktemp("n2-cut35")
    with ThreadPoolExecutor(max_workers=WORKERS) as pool:
        return tuple(
            pool.map(
                lambda pair: audit(pair[1], workspace / f"arm{pair[0]}"),
                enumerate(CUT35_ARMS),
            )
        )


def test_the_inventory_is_exactly_the_twenty_seven_declared_units() -> None:
    ...
    assert len(CUT35_ARMS) == 28 and len(DECLARATION_UNITS) == 27
    assert all(arm.sabotage.package == "beliefs" for arm in CUT35_ARMS)
    assert set(UNIT_CHECKS) == set(DECLARATION_UNITS)
    assert {check for arm in CUT35_ARMS for check in arm.checks} == set(UNIT_CHECKS.values())
    homed = {unit: sum(unit_of(arm.row) == unit for arm in CUT35_ARMS) for unit in DECLARATION_UNITS}
    assert homed == {unit: 2 if unit == "BI-11" else 1 for unit in DECLARATION_UNITS}


def test_each_arm_is_unique_and_carries_an_exact_check() -> None:
    rows = [arm.row for arm in CUT35_ARMS]
    assert len(rows) == len(set(rows))
    befores = [arm.sabotage.before for arm in CUT35_ARMS]
    assert len(befores) == len(set(befores))
    for arm in CUT35_ARMS:
...
def test_every_acceptance_test_the_arms_name_exists() -> None:
    module = REPO_ROOT / "python" / "tests" / "acceptance" / "test_url_retrieval_acceptance.py"
    names = {
        node.name
...
def test_every_live_check_resolves_and_passes_without_sabotage() -> None:
    every = Arm(
        row="N2",
        asserts="every cut-35 check passes against the real package",
        sabotage=CUT35_ARMS[0].sabotage,
        checks=tuple(dict.fromkeys(check for arm in CUT35_ARMS for check in arm.checks)),
    )
...
def test_the_freeze_commit_and_sections_two_through_seven_are_pinned() -> None:
    ...
    assert (
        subprocess.run(
            ["git", "-C", str(REPO_ROOT), "merge-base", "--is-ancestor", CUT35_FREEZE_COMMIT, "HEAD"],
            check=False,
        ).returncode
        == 0
    ), CUT35_FREEZE_COMMIT
    current = FROZEN_CUT.read_text(encoding="utf-8")
    frozen = _show(CUT35_FREEZE_COMMIT, str(FROZEN_CUT.relative_to(REPO_ROOT)))
    assert sha256(frozen.encode("utf-8")).hexdigest() == CUT35_FROZEN_SHA256
    assert _frozen_body(current) == _frozen_body(frozen)
    assert "**27 declaration units**" in current
    assert '("cut34_acceptance.py",)' in current
...
def test_the_declaration_is_byte_exact_against_its_pinned_digest() -> None:
    ...
    assert sha256(current).hexdigest() == CUT35_DECLARATION_SHA256
    shim = (REPO_ROOT / "python" / "tests" / "acceptance" / "n2_arms_cut35.py").read_text(encoding="utf-8")
    assert "n2_arms_cut35.py" in shim  # the acceptance copy re-exports, never restates
...
def test_row_parser_accepts_only_exact_declared_units() -> None:
    ...
    assert unit_of("BI-11a") == unit_of("BI-11b") == "BI-11"
    for row in ("", "H4", "H4-c", "H4-aa", "BI", "BI-12", "BI-1a", "BI-1-a", "BI-10a", "C8-a", "U1"):
        with pytest.raises(ValueError, match="is not a cut-35 row"):
            unit_of(row)
