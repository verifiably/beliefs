"""Cut 41 declaration accounting, freeze pin, and N2 audit."""
...
FROZEN_CUT = REPO_ROOT / "docs" / "designs" / "2026-09-25-conformance-cut-41.md"
CUT41_FREEZE_COMMIT = "e8d47f2e576e8c96a67e0898667acbeddd3dc6b2"
CUT41_FROZEN_SHA256 = "6e5374104758ebfc3c612166134397cbdb5d9946ff53afb8d99c949af16d2987"
FROZEN_DECLARATION = "python/tests/n2_arms_cut41.py"
CUT41_DECLARATION_SHA256 = "2437a3345c145d0e60a2122efc4d6538f028b513c9a008ec19061baafbf46f14"

# The frozen cut's §4: every Task 0 engine probe holds, so no arm is unrun.
FROZEN_ARMS, FROZEN_UNITS = 12, 12
...
def findings(tmp_path_factory):
    workspace = tmp_path_factory.mktemp("n2-cut41")
    with ThreadPoolExecutor(max_workers=WORKERS) as pool:
        return tuple(
            pool.map(
                lambda pair: audit(pair[1], workspace / f"arm{pair[0]}"),
                enumerate(CUT41_ARMS),
            )
        )
...
def test_the_inventory_is_exactly_the_frozen_rows_units() -> None:
    ...
    assert (FROZEN_ARMS, FROZEN_UNITS) == (12, 12)
    assert len(CUT41_ARMS) == FROZEN_ARMS and len(DECLARATION_UNITS) == FROZEN_UNITS
    assert all(arm.sabotage.package == "beliefs" for arm in CUT41_ARMS)
    assert set(UNIT_CHECKS) == set(DECLARATION_UNITS)
    assert {check for arm in CUT41_ARMS for check in arm.checks} == set(UNIT_CHECKS.values())
    homed = {unit: sum(unit_of(arm.row) == unit for arm in CUT41_ARMS) for unit in DECLARATION_UNITS}
    assert homed == dict.fromkeys(DECLARATION_UNITS, 1)


def test_each_arm_is_unique_and_carries_an_exact_check() -> None:
    rows = [arm.row for arm in CUT41_ARMS]
    assert len(rows) == len(set(rows))
    befores = [arm.sabotage.before for arm in CUT41_ARMS]
    assert len(befores) == len(set(befores))
    for arm in CUT41_ARMS:
...
def test_every_acceptance_test_the_arms_name_exists() -> None:
    ...
    module = REPO_ROOT / "python" / "tests" / "acceptance" / "test_live_selection_acceptance.py"
    names = {
        node.name
...
def test_every_live_check_resolves_and_passes_without_sabotage() -> None:
    every = Arm(
        row="N2",
        asserts="every cut-41 check passes against the real package",
        sabotage=CUT41_ARMS[0].sabotage,
        checks=tuple(dict.fromkeys(check for arm in CUT41_ARMS for check in arm.checks)),
    )
...
def test_the_freeze_commit_and_sections_two_through_seven_are_pinned() -> None:
    ...
    assert (
        subprocess.run(
            ["git", "-C", str(REPO_ROOT), "merge-base", "--is-ancestor", CUT41_FREEZE_COMMIT, "HEAD"],
            check=False,
        ).returncode
        == 0
    ), CUT41_FREEZE_COMMIT
    current = FROZEN_CUT.read_text(encoding="utf-8")
    frozen = _show(CUT41_FREEZE_COMMIT, str(FROZEN_CUT.relative_to(REPO_ROOT)))
    assert sha256(frozen.encode("utf-8")).hexdigest() == CUT41_FROZEN_SHA256
    assert _frozen_body(current) == _frozen_body(frozen)
    # The frozen §4 wraps the bold accounting across a line break; compare on normalized whitespace.
    assert "**12 arms, 12 declaration units**" in " ".join(current.split())
    assert '("cut40_acceptance.py",)' in current
...
def test_the_declaration_is_byte_exact_against_its_pinned_digest() -> None:
    ...
    assert sha256(current).hexdigest() == CUT41_DECLARATION_SHA256
    shim = (REPO_ROOT / "python" / "tests" / "acceptance" / "n2_arms_cut41.py").read_text(encoding="utf-8")
    assert "n2_arms_cut41.py" in shim  # the acceptance copy re-exports, never restates
...
def test_row_parser_accepts_only_exact_declared_units() -> None:
    ...
    for row in (
        ...
        with pytest.raises(ValueError, match="is not a cut-41 row"):
            unit_of(row)
