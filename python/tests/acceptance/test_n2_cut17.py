"""Cut 17 declaration accounting, freeze pin, and N2 audit."""
...
from n2_arms_cut17 import CO_CITED, CUT17_ARMS, DECLARATION_UNITS, unit_of
...
FROZEN_CUT = REPO_ROOT / "docs" / "designs" / "2026-09-04-conformance-cut-17.md"
CUT17_FREEZE_COMMIT = "2071be0"
CUT17_FROZEN_SHA256 = "797775b5c591c3c7ade1d1e4f0f188097a4fac27f21435218dbb4fa7f8e54e29"
...
def findings(tmp_path_factory):
    workspace = tmp_path_factory.mktemp("n2-cut17")
    with ThreadPoolExecutor(max_workers=WORKERS) as pool:
        return tuple(
            pool.map(
                lambda pair: audit(pair[1], workspace / f"arm{pair[0]}"),
                enumerate(CUT17_ARMS),
            )
        )
...
def test_the_inventory_is_exactly_the_seventeen_frozen_units() -> None:
    ...
    assert {unit_of(arm.row) for arm in CUT17_ARMS} == set(DECLARATION_UNITS)
...
def test_the_freeze_commit_and_sections_two_through_seven_are_pinned() -> None:
    assert (
        subprocess.run(
            ["git", "-C", str(REPO_ROOT), "merge-base", "--is-ancestor", CUT17_FREEZE_COMMIT, "HEAD"],
            check=False,
        ).returncode
        == 0
    )
    ...
    frozen = subprocess.run(
        [
            ...
            str(REPO_ROOT),
            "show",
            f"{CUT17_FREEZE_COMMIT}:{FROZEN_CUT.relative_to(REPO_ROOT)}",
        ],
        check=True,
        capture_output=True,
        text=True,
    ).stdout
    assert sha256(frozen.encode("utf-8")).hexdigest() == CUT17_FROZEN_SHA256
