"""Cut 33 declaration accounting, freeze pin, and N2 audit."""
...
from n2_arms_cut33 import CO_CITED, CUT33_ARMS, DECLARATION_UNITS, UNIT_CHECKS, unit_of
...
FROZEN_CUT = REPO_ROOT / "docs" / "designs" / "2026-09-16-conformance-cut-33.md"
CUT33_FREEZE_COMMIT = "23f8a213737ae053c2a7715779e033c5802e20df"
CUT33_FROZEN_SHA256 = "636b9b253507ad7f5eb4bfcdce560b7efe708f0b690bfc027415b7c16301a9b6"
FROZEN_DECLARATION = "python/tests/n2_arms_cut33.py"
CUT33_DECLARATION_SHA256 = "abf0e8289065634c4d355e080b716fbfeb4d7c2f276c0411b01a48bc11c7569d"
...
def findings(tmp_path_factory):
    workspace = tmp_path_factory.mktemp("n2-cut33")
    with ThreadPoolExecutor(max_workers=WORKERS) as pool:
        return tuple(
            pool.map(
                lambda pair: audit(pair[1], workspace / f"arm{pair[0]}"),
                enumerate(CUT33_ARMS),
            )
        )
...
def test_the_inventory_is_exactly_the_eleven_declared_units() -> None:
    ...
    assert len(CUT33_ARMS) == len(DECLARATION_UNITS) == 11
    assert all(arm.sabotage.package == "beliefs" for arm in CUT33_ARMS)
    assert set(UNIT_CHECKS) == set(DECLARATION_UNITS)
    assert {check for arm in CUT33_ARMS for check in arm.checks} == set(UNIT_CHECKS.values())
    homed = {unit: sum(unit_of(arm.row) == unit for arm in CUT33_ARMS) for unit in DECLARATION_UNITS}
    assert homed == {unit: 1 for unit in DECLARATION_UNITS}


def test_each_arm_is_unique_and_carries_an_exact_check() -> None:
    rows = [arm.row for arm in CUT33_ARMS]
    assert len(rows) == len(set(rows))
    for arm in CUT33_ARMS:
...
def test_every_acceptance_test_the_arms_name_exists() -> None:
    module = REPO_ROOT / "python" / "tests" / "acceptance" / "test_correction_acceptance.py"
    names = {
        node.name
...
def test_every_live_check_resolves_and_passes_without_sabotage() -> None:
    every = Arm(
        row="N2",
        asserts="every cut-33 check passes against the real package",
        sabotage=CUT33_ARMS[0].sabotage,
        checks=tuple(dict.fromkeys(check for arm in CUT33_ARMS for check in arm.checks)),
    )
...
def test_the_freeze_commit_and_sections_two_through_seven_are_pinned() -> None:
    ...
    assert (
        subprocess.run(
            ["git", "-C", str(REPO_ROOT), "merge-base", "--is-ancestor", CUT33_FREEZE_COMMIT, "HEAD"],
            check=False,
        ).returncode
        == 0
    ), CUT33_FREEZE_COMMIT
    current = FROZEN_CUT.read_text(encoding="utf-8")
    frozen = _show(CUT33_FREEZE_COMMIT, str(FROZEN_CUT.relative_to(REPO_ROOT)))
    assert sha256(frozen.encode("utf-8")).hexdigest() == CUT33_FROZEN_SHA256
    assert _frozen_body(current) == _frozen_body(frozen)
    assert "**11 declaration units**" in current
    assert '("cut32_acceptance.py",)' in current
...
def test_the_declaration_is_byte_exact_against_its_pinned_digest() -> None:
    ...
    assert sha256(current).hexdigest() == CUT33_DECLARATION_SHA256
    shim = (REPO_ROOT / "python" / "tests" / "acceptance" / "n2_arms_cut33.py").read_text(encoding="utf-8")
    assert "n2_arms_cut33.py" in shim  # the acceptance copy re-exports, never restates
...
def test_row_parser_accepts_only_exact_declared_units() -> None:
    for unit in DECLARATION_UNITS:
        ...
        with pytest.raises(ValueError, match="is not a cut-33 row"):
            unit_of(f"{unit}-a")
    for row in ("", "C7", "C7-d", "BI", "BI-6", "BI-1-a", "U1"):
        with pytest.raises(ValueError, match="is not a cut-33 row"):
            unit_of(row)
