"""Cut 24 declaration accounting, freeze pin, and N2 audit."""
...
from n2_arms_cut24 import CO_CITED, CUT24_ARMS, DECLARATION_UNITS, unit_of
...
FROZEN_CUT = REPO_ROOT / "docs" / "designs" / "2026-09-10-conformance-cut-24.md"
CUT24_FREEZE_COMMIT = "c72219908c405fb8ccd5d7541c2f588407e89314"
CUT24_FROZEN_SHA256 = "e15ff057eca27c21907d9c7288811b5c50b34c1041dc038f5c03f54c8e8b5798"
...
def findings(tmp_path_factory):
    workspace = tmp_path_factory.mktemp("n2-cut24")
    with ThreadPoolExecutor(max_workers=WORKERS) as pool:
        return tuple(
            pool.map(
                lambda pair: audit(pair[1], workspace / f"arm{pair[0]}"),
                enumerate(CUT24_ARMS),
            )
        )
...
def test_each_lettered_arm_is_unique_and_carries_an_exact_check() -> None:
    rows = [arm.row for arm in CUT24_ARMS]
    assert len(rows) == len(set(rows))
    for arm in CUT24_ARMS:
...
def test_every_declared_check_resolves_and_passes_without_sabotage() -> None:
    every = Arm(
        row="N2",
        asserts="every cut-24 check passes against the real package",
        sabotage=CUT24_ARMS[0].sabotage,
        checks=tuple(dict.fromkeys(check for arm in CUT24_ARMS for check in arm.checks)),
    )
...
def test_the_freeze_commit_and_sections_two_through_seven_are_pinned() -> None:
    ...
    assert (
        subprocess.run(
            ["git", "-C", str(REPO_ROOT), "merge-base", "--is-ancestor", CUT24_FREEZE_COMMIT, "HEAD"],
            check=False,
        ).returncode
        == 0
    ), CUT24_FREEZE_COMMIT
    current = FROZEN_CUT.read_text(encoding="utf-8")
    frozen = _show(CUT24_FREEZE_COMMIT, str(FROZEN_CUT.relative_to(REPO_ROOT)))
    assert sha256(frozen.encode("utf-8")).hexdigest() == CUT24_FROZEN_SHA256
    assert _frozen_body(current) == _frozen_body(frozen)
    assert "**5 declaration units**" in current
    assert "Five guarantee rows are read, **2 full/closed** (W15, W4), 3 partial" in current
    assert '("cut23_acceptance.py",)' in current
