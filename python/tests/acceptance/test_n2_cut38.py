"""Cut 38 declaration accounting, freeze pin, and N2 audit."""
...
from n2_arms_cut38 import CO_CITED, CUT38_ARMS, DECLARATION_UNITS, UNIT_CHECKS, unit_of
...
FROZEN_CUT = REPO_ROOT / "docs" / "designs" / "2026-09-22-conformance-cut-38.md"
CUT38_FREEZE_COMMIT = "21ef347a0a7dbfd4acabeb996d17663384892dfa"
CUT38_FROZEN_SHA256 = "4b06f8fe9d310bfde6766c7b9328575d0cc758d299f54ec493b783cf1214efc6"
FROZEN_DECLARATION = "python/tests/n2_arms_cut38.py"
CUT38_DECLARATION_SHA256 = "b3953f933ba06782d91153e447e3ee8dc1c50c7d97bfd71cd1a7dfa90eed380b"
...
def findings(tmp_path_factory):
    workspace = tmp_path_factory.mktemp("n2-cut38")
    with ThreadPoolExecutor(max_workers=WORKERS) as pool:
        return tuple(
            pool.map(
                lambda pair: audit(pair[1], workspace / f"arm{pair[0]}"),
                enumerate(CUT38_ARMS),
            )
        )


def test_the_inventory_is_exactly_the_twelve_declared_units() -> None:
    ...
    assert len(CUT38_ARMS) == 14 and len(DECLARATION_UNITS) == 12
    assert all(arm.sabotage.package == "beliefs" for arm in CUT38_ARMS)
    assert set(UNIT_CHECKS) == set(DECLARATION_UNITS)
    assert {check for arm in CUT38_ARMS for check in arm.checks} == set(UNIT_CHECKS.values())
    homed = {unit: sum(unit_of(arm.row) == unit for arm in CUT38_ARMS) for unit in DECLARATION_UNITS}
    assert homed == {unit: {"T2-g": 3}.get(unit, 1) for unit in DECLARATION_UNITS}


def test_each_arm_is_unique_and_carries_an_exact_check() -> None:
    rows = [arm.row for arm in CUT38_ARMS]
    assert len(rows) == len(set(rows))
    befores = [arm.sabotage.before for arm in CUT38_ARMS]
    assert len(befores) == len(set(befores))
    for arm in CUT38_ARMS:
...
def test_every_acceptance_test_the_arms_name_exists() -> None:
    ...
    Not `len(names) == len(DECLARATION_UNITS)`, the earlier cuts' spelling: this
    module carries three plain tests besides the twelve units, for the already-closed
    rows the new kinds exercise. Nor a prefix filter over them — `test_the_two_reports_…`
    begins with `test_t` exactly as a unit's test does.
    """
    module = REPO_ROOT / "python" / "tests" / "acceptance" / "test_act_report_remainder_acceptance.py"
    names = {
        node.name
...
def test_every_live_check_resolves_and_passes_without_sabotage() -> None:
    every = Arm(
        row="N2",
        asserts="every cut-38 check passes against the real package",
        sabotage=CUT38_ARMS[0].sabotage,
        checks=tuple(dict.fromkeys(check for arm in CUT38_ARMS for check in arm.checks)),
    )
...
def test_the_freeze_commit_and_sections_two_through_seven_are_pinned() -> None:
    ...
    assert (
        subprocess.run(
            ["git", "-C", str(REPO_ROOT), "merge-base", "--is-ancestor", CUT38_FREEZE_COMMIT, "HEAD"],
            check=False,
        ).returncode
        == 0
    ), CUT38_FREEZE_COMMIT
    current = FROZEN_CUT.read_text(encoding="utf-8")
    frozen = _show(CUT38_FREEZE_COMMIT, str(FROZEN_CUT.relative_to(REPO_ROOT)))
    assert sha256(frozen.encode("utf-8")).hexdigest() == CUT38_FROZEN_SHA256
    assert _frozen_body(current) == _frozen_body(frozen)
    assert "**12 declaration units**" in current
    assert '("cut37_acceptance.py",)' in current
...
def test_the_declaration_is_byte_exact_against_its_pinned_digest() -> None:
    ...
    assert sha256(current).hexdigest() == CUT38_DECLARATION_SHA256
    shim = (REPO_ROOT / "python" / "tests" / "acceptance" / "n2_arms_cut38.py").read_text(encoding="utf-8")
    assert "n2_arms_cut38.py" in shim  # the acceptance copy re-exports, never restates
...
def test_row_parser_accepts_only_exact_declared_units() -> None:
    ...
    assert unit_of("T2-g1") == unit_of("T2-g2") == unit_of("T2-g3") == "T2-g"
    for row in (
        "",
        "T2",
        "T2-a",
        ...
        with pytest.raises(ValueError, match="is not a cut-38 row"):
            unit_of(row)
