"""Cut 37 declaration accounting, freeze pin, and N2 audit."""
...
from n2_arms_cut37 import CO_CITED, CUT37_ARMS, DECLARATION_UNITS, UNIT_CHECKS, unit_of
...
FROZEN_CUT = REPO_ROOT / "docs" / "designs" / "2026-09-21-conformance-cut-37.md"
CUT37_FREEZE_COMMIT = "b4f27a26c552a03bbe2371175cacf739385e905e"
CUT37_FROZEN_SHA256 = "9d43fa5c1a559d064b8116cf6c0d1395343e6f4bb110603a64b4704b5abe8292"
FROZEN_DECLARATION = "python/tests/n2_arms_cut37.py"
CUT37_DECLARATION_SHA256 = "2af3031429451082791ea905083de858971862b29257b0c97838a946cf5b9cfa"
...
def findings(tmp_path_factory):
    workspace = tmp_path_factory.mktemp("n2-cut37")
    with ThreadPoolExecutor(max_workers=WORKERS) as pool:
        return tuple(
            pool.map(
                lambda pair: audit(pair[1], workspace / f"arm{pair[0]}"),
                enumerate(CUT37_ARMS),
            )
        )


def test_the_inventory_is_exactly_the_eleven_declared_units() -> None:
    ...
    assert len(CUT37_ARMS) == 15 and len(DECLARATION_UNITS) == 11
    assert all(arm.sabotage.package == "beliefs" for arm in CUT37_ARMS)
    assert set(UNIT_CHECKS) == set(DECLARATION_UNITS)
    assert {check for arm in CUT37_ARMS for check in arm.checks} == set(UNIT_CHECKS.values())
    homed = {unit: sum(unit_of(arm.row) == unit for arm in CUT37_ARMS) for unit in DECLARATION_UNITS}
    assert homed == {unit: {"L13-b": 2, "L13-d": 4}.get(unit, 1) for unit in DECLARATION_UNITS}


def test_each_arm_is_unique_and_carries_an_exact_check() -> None:
    rows = [arm.row for arm in CUT37_ARMS]
    assert len(rows) == len(set(rows))
    befores = [arm.sabotage.before for arm in CUT37_ARMS]
    assert len(befores) == len(set(befores))
    for arm in CUT37_ARMS:
...
def test_every_acceptance_test_the_arms_name_exists() -> None:
    module = REPO_ROOT / "python" / "tests" / "acceptance" / "test_l13_preimage_acceptance.py"
    names = {
        node.name
...
def test_every_live_check_resolves_and_passes_without_sabotage() -> None:
    every = Arm(
        row="N2",
        asserts="every cut-37 check passes against the real package",
        sabotage=CUT37_ARMS[0].sabotage,
        checks=tuple(dict.fromkeys(check for arm in CUT37_ARMS for check in arm.checks)),
    )
...
def test_the_freeze_commit_and_sections_two_through_seven_are_pinned() -> None:
    ...
    assert (
        subprocess.run(
            ["git", "-C", str(REPO_ROOT), "merge-base", "--is-ancestor", CUT37_FREEZE_COMMIT, "HEAD"],
            check=False,
        ).returncode
        == 0
    ), CUT37_FREEZE_COMMIT
    current = FROZEN_CUT.read_text(encoding="utf-8")
    frozen = _show(CUT37_FREEZE_COMMIT, str(FROZEN_CUT.relative_to(REPO_ROOT)))
    assert sha256(frozen.encode("utf-8")).hexdigest() == CUT37_FROZEN_SHA256
    assert _frozen_body(current) == _frozen_body(frozen)
    assert "**11 declaration units**" in current
    assert '("cut36_acceptance.py",)' in current
...
def test_the_declaration_is_byte_exact_against_its_pinned_digest() -> None:
    ...
    assert sha256(current).hexdigest() == CUT37_DECLARATION_SHA256
    shim = (REPO_ROOT / "python" / "tests" / "acceptance" / "n2_arms_cut37.py").read_text(encoding="utf-8")
    assert "n2_arms_cut37.py" in shim  # the acceptance copy re-exports, never restates
...
def test_row_parser_accepts_only_exact_declared_units() -> None:
    ...
    assert unit_of("L13-b1") == unit_of("L13-b2") == "L13-b"
    assert unit_of("L13-d1") == unit_of("L13-d2") == unit_of("L13-d3") == unit_of("L13-d4") == "L13-d"
    for row in (
        "",
        "L13",
        "L13-i",
        ...
        with pytest.raises(ValueError, match="is not a cut-37 row"):
            unit_of(row)
