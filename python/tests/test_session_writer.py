...
def test_the_scoped_writer_exposes_the_eight_methods_the_routes_and_its_invocation(tmp_path):
    public = {name for name in dir(ScopedWriter) if not name.startswith("_")}
    assert public == {
        "add", "retract", "attest_coreference", "supersede", "revise", "delete", "mint_coordination", "revise_coordination",
