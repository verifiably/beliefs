...
**Status:** pre-integration migration in progress; documentation has been reconciled and task creation is pending.
...
| The active cut-13 design and plan used the pre-rename executable package path and import name. | The tree, package metadata, imports, and tests use `python/src/beliefs` and `beliefs.*`; the rename is in base history. | Executable source paths, module invocations, imports, and test module names in those two active documents now use `beliefs`. Frozen identity domains such as `science.environment.v2`, `science.boundary-receipt.v2`, `science.run.v2`, and sandbox paths under `/science/` remain unchanged. | Negative searches find no `python/src/science`, `src/science`, `from science.`, or `import science.` in the active pair, and find no accidental `beliefs.*.vN` identity replacement. Historical plans and cut evidence were not rewritten. |
...
| Deliver run confinement and discharge cut 13 | Tier-1 boundary is fully designed and planned but absent from code. | Roadmap `run-confinement`; run-confinement design and plan | No active branch or owner | xl | todo | none | create | pending CLI creation |
| Deliver the full workflow surface | Cut 3 explicitly left multi-rule, family, wildcard, and definition-equality workflows outside the minimal adapter. | Roadmap `workflow-surface`; computation design | No active branch or owner | xl | todo | run confinement, by strict execution-lane order | create | pending CLI creation |
| Deliver consolidate, move, and managed deletion | The roadmap groups the last mutation family with run-boundary and formal-model ride-alongs. | Roadmap `consolidate-family`; family-adapters and correction designs | No active branch or owner | xl | todo | none | create | pending CLI creation |
| Deliver URL acquisition and act-report coverage | URL locators and remote acquisition remain open; act-report remainders explicitly ride with the boundary. | Roadmap `url-retrieval`; holdings and act-report designs | No active branch or owner | l | todo | none | create | pending CLI creation |
| Deliver world resolution and packaging remainder | The write boundary and index have landed; the read-side resolution rows and packaging ride-along remain open. | Roadmap `world-resolution`; world addressing and packaging designs | No active branch or owner | xl | todo | none | create | pending CLI creation |
| Deliver the domain boundary, biology pack, and second parity fixture | Domain kinds remain absent; the approved biology pack and second fixture ride with this boundary. | Roadmap `domain-boundary`; domain design; user/autonomy design §8 | No active branch or owner | xl | todo | none | create | pending CLI creation |
| Deliver event-level L8 and the log remainder | The ordered-cuts predicate exists; event-level relation and log relabels remain. | Roadmap `event-level-l8`; log design | No active branch or owner | l | todo | world resolution, by strict world-read-lane order | create | pending CLI creation |
| Deliver the first full contract cut | Contract identity, certification, legacy-check, and rules-store rows remain; roadmap makes this the post-lane join. | Roadmap `contract-cut`; normative contract design | No active branch or owner | xl | todo | workflow surface; consolidate family; URL acquisition; event-level L8; domain boundary | create | pending CLI creation |
| Complete the correction lifecycle | C7–C10 remainders remain and C7 requires the consolidate surface. | Roadmap `correction-remainder`; correction lifecycle design | No active branch or owner | xl | todo | consolidate family | create | pending CLI creation |
| Close L13 through a public preimage seam | L13 needs blob bytes rather than only a path; Atoms owns the audited seam. | Roadmap `l13-preimage`; Atoms task `atoms-38887b` | No active branch or owner | m | todo | `atoms-38887b` | create | pending CLI creation |
| Certify publication persistence | X2 needs Atoms A8 extended through the publication path. | Roadmap `persistence-cut`; Atoms task `atoms-f5779f` | No active branch or owner | l | todo | `atoms-f5779f` | create | pending CLI creation |
...
| Design authority snapshot labels | W9/W14 remain blocked on authority snapshot governance. | Roadmap `authority-labels`; open questions | No active branch or owner | l | idea | unresolved design evidence, not a task edge | create | pending CLI creation |
...
| Deliver coordination and view kinds | The approved umbrella design specifies the governed kinds and guarantee rows, while the existing task owns the design that must land first. | User/autonomy design §§4.1–4.2, §8; roadmap `coordination-addressing` | No active branch or owner | xl | todo | `beliefs-c88566` | create | pending CLI creation |
| Design weighted-belief semantics | S6(h) awaits an estimand-typing and weighting ruling. | Roadmap `weighted-belief`; belief policy; open questions | No active branch or owner | l | idea | unresolved design evidence, not a task edge | create | pending CLI creation |
| Design the higher-order extraction path | M12 awaits a ruling for extraction and higher-order records. | Roadmap `extraction-path`; formal-model design; open questions | No active branch or owner | l | idea | unresolved design evidence, not a task edge | create | pending CLI creation |
| Design cross-root publication residue | T7's cross-root case remains an explicit design question. | Roadmap `cross-root-publication`; act-report design; open questions | No active branch or owner | l | idea | unresolved design evidence, not a task edge | create | pending CLI creation |
| Enforce write permits at every Beliefs write entry point | The approved command-framework design requires permits in the kernel writer, family adapters, run boundary, and publish path; none exists in code. | User/autonomy design §5.2 and §8 item 2 | No active branch or owner | xl | todo | none | create | pending CLI creation |
| Implement the Beliefs publish act and governed records | The approved design specifies the act, publication marker/binding, recovery, and act-report amendment; no publish entry point or record kinds exist. | User/autonomy design §§4.1, 6, and §8 item 5 | No active branch or owner | xl | todo | coordination/view delivery; event-level L8 (completion of the world-read lane) | create | pending CLI creation |
...
Every `create` row will use the `migration` tag plus one stable lane or concern tag. The corresponding body will state the outcome, acceptance evidence, sources, and uncertainty. The four tier-3 design decisions are `idea`; unfinished designed delivery is `todo`; no `doing` or `blocked` state is inferred from branch names or dependencies.
...
Uncertainty: The roadmap fixes the join point, but successor identities, certification cadence, normative artifact shape, and legacy-check ruling need their design cycle.
...
None. The only verified foreign blockers of created Beliefs tasks are the already-integrated, resolvable Atoms tasks `atoms-38887b` and `atoms-f5779f`. The Nodes remainder is producer-owned rather than a Beliefs delivery blocker, and the audited evidence establishes no dependency on a future Nodes or Mindful task. If later evidence establishes such an edge, it must remain `pending` in this section until that producer task exists; no dangling CLI edge is permitted.
...
| Temporary-registry `tasks check`, exact existing-record comparison, and created-task field review | Pending task checkpoint. | Pending |
