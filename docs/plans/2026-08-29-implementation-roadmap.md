...
| `acquisition` | none — `url-retrieval` closed at cut 35; its ride-along `act-report-remainder` re-homed to `world-read` with T2's remainder | `holdings/`, `report.py`, `corpus.py`, `session/writer.py`, `intents/`, `stored.py`; the overlap with `world-read` on `corpus.py` and `world/verify.py` was named at the freeze under rule 3 | closed 2026-09-20 at cut 35 |
...
| T2 | the `audit` and `re-check` operation kinds, which no boundary opens as an operation (cut 35 results §2, url-retrieval design decision 12); every other kind — `run-attempt`, `import`, `move`, `consolidate`, `corpus-write`, `acquisition` — is read to success, with root-selection failure, intent-append failure and the second fulfillment read at cut 35; `delete` contributes no arm (cut 18 §4) | `act-report-remainder` — tier 1, off the path |
