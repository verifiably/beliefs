...
**Ranked at:** cut 39, against the ledger's Current state (2026-09-23)
...
the act-report's `publish` kind, and its second slice's lifecycle entries
...
## Appendix A — live status of every guarantee row at cut 39

Produced by `python/tools/roadmap_status.py` from the cuts' own accounting
(spec §3.1); a row is closed only when no later source reopens it. Cut 39
closes W17 and Y1–Y4 in full, reads no row in part, and reopens nothing. L1
