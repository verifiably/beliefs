...
historical evidence. At that feature-branch checkpoint, main landing and cut 21
remained pending; biology slice 2
remains open.


## Local main integration — 2026-09-07

The reviewed feature tip `fc09bd158d64b398505be390bc2cf958e95ee030`
lands with main tip `02f5243` in the local merge. This includes main’s
package-name, hooks, and CI updates that were outside the earlier feature
validation. The merge preserves main’s current repository guidance, the
explicit test-module loader refusal, and one Pyright tools-path setting.
The domain lane remains open for biology slice 2; its worktree and fresh
step-3 reproduction corpus are preserved. Cut 21 remains separately
undischarged. The earlier synchronization refusal and all preceding gate
results above remain historical evidence for their stated revisions.

Normal synchronized validation of this merged tree passed:

- `just test`: **4050 Python tests passed in 1037.54s**, serially, and
  **137 TypeScript tests passed**.
- `cd python && uv run --frozen python tools/cut20_acceptance.py`: the
  complete certified prefix and final **17 durable + 5 N2/inventory/freeze
  checks** passed in 1165.48s. Accounting remains **12 sabotage arms /
  18 declaration units**, with 15 full/closed rows and D1 partial.
- `just check`: Ruff, Pyright, TypeScript and Biome passed; `tasks check`
  reported zero errors and warnings.

These runs used normal frozen dependency synchronization, with `UV_NO_SYNC`,
`VERIFIABLY_UNCERTIFIED_HOST` and `BELIEFS_UNCERTIFIED_HOST` unset. The earlier
package-name synchronization block is resolved by the merged main updates.
The certified tuple remained kernel `7.2.2-arch1-1`, ext4
`rw,noatime,data=ordered`. Tracked source, HEAD and external dependency runtime
