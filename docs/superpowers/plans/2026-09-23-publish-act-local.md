...
2. **A destination path that is missing, a file, the operations root or inside it, inside a corpus root, or a symlink.** Each unusable one must refuse `destination-unusable` before the intent, rather than letting `replicate_root` fail after the intent. A symlinked destination is not unusable: it is accepted, and step 0 freezes its resolved target before the intent. Pinned by `test_destination_checks` in Task 4.
...
4. **A resume whose caller holds another actor, or whose `staging_profile` disagrees with the request's pins.** Both must refuse with nothing written, not as a terminal report. Pinned by `test_resume_refuses_another_actor_and_another_profile_writing_nothing` in Task 8 (it needs a real chain; Task 6 Step 1 defers it there).
