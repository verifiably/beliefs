...
- No new dependency, closure-result cache or file-digest cache. Reusing a resolved parent only within one capture is allowed; every capture still hashes every file. Keep the identity memo outside `EnvironmentManifest` fields, `vars()`, repr, equality and pickle; bound it to four entries.
...
- [x] **Step 1: Write one failing test in `test_recipe.py`.** Construct two equal manifests and one with a changed digest row. Before spying, call `_environment_identity.cache_clear()` if the helper exists, so a prior test on the same worker cannot affect the count; this conditional permits a red run before the helper is added. Spy on `v1.digest`: the equal pair must invoke it once; the changed row must yield a different identity and a second digest call. After warming, assert `dataclasses.fields` names only `artifacts`, `vars(manifest)` still contains only `artifacts`, repr is unchanged, equality holds, and pickle round-trips.
- [x] **Step 2: Run the focused test before implementation.** From `python/`, run `uv run --frozen pytest tests/test_recipe.py::<new-test-name>`; it must fail on the equal-pair digest count of two, not on setup or an unrelated assertion.
- [x] **Step 3: Add the pure helper in `recipe.py`.** Import `functools.lru_cache`, decorate `_environment_identity` with `maxsize=4`, pass `self.artifacts`, and keep the existing `v1.digest(ENVIRONMENT_DOMAIN, {"artifacts": _triples(artifacts)})` calculation and public method return value. Do not store anything on the dataclass or cache capture results.
- [x] **Step 4: Verify the behavior and gain.** Run the new test, `test_closure_capture.py`, `test_frozen_guards.py`, and `test_arm_staleness.py`, then time repeated calls on one manifest and a representative replay/boundary pipeline test. No frozen pin names `recipe.py` or `test_recipe.py`, and no N2 arm anchors on the body of `EnvironmentManifest.identity`; the two guard suites check this after the edit. Run `just test-fast` once for a complete verdict. Compare worker-seconds and closure-call counts with Task 1 at the same host budget in Task 4; if the host budget differs now, record that limit rather than presenting the wall times as comparable. Revert the memo if the comparable end-to-end gain is within measurement noise or any identity/refusal changes.
- [x] **Step 5: Verify and commit.** Run `just check`, `tasks check` and `git diff --check`; close the Task 2 child with `tasks done` and commit the accepted memo, test and task record as `perf: memoize environment identity`, with measured evidence in the P0 task note.
...
- [x] **Step 1: Run the complete checks.** `just check` passed. Three certified 16-worker `test-fast` runs passed 5,709 tests with one skip each in 86.936, 87.167 and 87.424s by `tt`; two full gates each passed 5,910 executed tests with one Python skip in 235.749 and 237.467s. Standalone N2 took 148.54 and 150.75s. A diagnostic fast run measured 1,127 worker-seconds and 256 closure walks; the same scheduler before the path change used 1,310 worker-seconds. The earlier `loadfile` capture count of 188 reflects fewer fixture copies.
- [x] **Step 2: Apply the acceptance arithmetic.** The fast median is 87.167s ≤90s, and both accepted full runs are ≤300s. Serial collection equals the non-N2 plus N2 union; certified capability checks remained in the full gate. The measured remaining tail was per-file path work in closure capture, so `add_tree` now uses components from paths produced by `rglob`, and each `_Closure` resolves a repeated parent once per walk. Three direct captures of the same 8,465 rows took 1.454–1.493s, down from about 2.0s. Every capture still rehashes all files.
- [x] **Step 3: Finish the record.** Current-facing timing claims reflect the final runs. `tasks check` passed with zero errors and warnings; Task 4 and `beliefs-9b248a` were closed with one-line outcomes for this final commit. `ops-5beefd` remains the separate cross-project policy task.
