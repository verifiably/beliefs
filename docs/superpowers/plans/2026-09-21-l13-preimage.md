...
| L13-e | L13 | corrupt local history: the indexed preimage leaf under the metadata root overwritten → the audit refuses `LogEvidenceRefused` with `phase == "preimage"` and `engine_error == "MetadataStoreInvalid"`, no report, every project file byte-identical |
...
§6 second reader: check that L13-c's hand-rendered copy really claims the removed path (same id) and really differs in digest; that L13-d's restored copy is `read-only-serviceable` (registered inspection) and the arrival's copy `metadata-less` (detached); that L13-e overwrites the leaf the store indexes under the removed digest (`blobs/sha256/<hex>`) and not some other file; that BI-1 observes the writer hold through `BuildContended` on the real operation lock and not a stub; that BI-3's stand-in replaces `read_preimage` only.
...
def test_failing_classification_resolves_through_history_naming_digest(tmp_path):
    ...
    view = cut8_view(chain)
    disk = cut8_disk(chain)
    removed_bytes = chain.contents[RECORD_PATH]
...
`chain.contents[RECORD_PATH]` presumes the `Chain` builder in `test_world_log_codecs.py` records the bytes it wrote; read `populated_corpus` and `Chain` there (`grep -n "class Chain" -A 40 tests/test_world_log_codecs.py`). If it records no bytes, read them **before** `removed_record` unlinks the file: build `populated_corpus(tmp_path)` first, `removed_bytes = (chain.root / RECORD_PATH).read_bytes()`, then apply the removal exactly as `removed_record` does (`state_at`, unlink, `chain.transaction("tx-3", ...)`, `chain.paths = (MANIFEST_PATH, RECORD_PATH)`) — copy those four lines rather than calling `removed_record`. Add a small helper `removals_txid(view)` returning the one committed removal's txid via `verify.committed_removals(view, ENGINE_ABSENT)[0].txid`. In `test_history_validation_refusals_and_digest_named_findings`, keep the validation assertions verbatim and change the tail so the classified case holds the **removed bytes** (`held(removed_bytes)`, code `failing-verification-removed`) and the `passed` copy reads `removal-unclassified` — every classification that *is* produced still names the digest it matched.
...
`_HeldRecord`, `_held_records`, `_record_path` and `_held_record` are gone. Add `PreimageMismatch` to the `from beliefs.errors import (...)` block; remove `NodeId` from the `nodes.core.ids` import if `grep -n "NodeId" src/beliefs/world/verify.py` now shows only the import (it will). Add to `__all__`: `"NO_PREIMAGES"`, `"PreimageEvidence"`, `"PreimageRead"`, `"PreimageUnavailable"`, `"Preimages"`, `"Removal"`, `"committed_removals"`, `"removed_digest"`.
...
and give `make_seam` two keyword parameters, `preimages: Preimages | None = None` and `state_facts=opaque_facts`, passing `read_preimage=preimages if preimages is not None else verify._unwired_read_preimage` and `state_facts=state_facts` to `verify.LogSeam(...)`. Give `audit(...)` a `preimages: Preimages | None = None` keyword forwarded to `make_seam`. `test_the_history_reaches_the_policy_pass` sets `FACT_BYTES[removed] = payload` before building the view (and `del FACT_BYTES[removed]` at the end) so the held copy resolves by digest.
...
def test_l13e_corrupt_local_history_refuses_the_act_and_touches_no_project_file_durably(lane):
    """L13-e. The indexed preimage leaf under the metadata root is overwritten:
    the audit refuses `LogEvidenceRefused(phase="preimage",
...
setsid nohup uv run --frozen python tools/cut37_acceptance.py > ~/d/beliefs/.work/acceptance/cut37-runner.log 2>&1 &
```
Read the log when it exits; expected tail: three `[cut37 phase n/3]` lines, `declared arms: 15 (= 11 declaration units; 1 guarantee rows)`, the rows-exercised line, exit 0. Every arm `sound`; every check `resolved`. A `stale` verdict means a `before` no longer matches — fix the declaration (Step 1), never the source.
...
export SCIENCE_MM30_ROOT=~/d/beliefs/.work/reproduction/mm30
for n in $(seq 4 37); do export SCIENCE_CUT${n}_ROOT=~/d/beliefs/.work/acceptance/cut$n; done
setsid nohup just gate > ~/d/beliefs/.work/acceptance/cut37-gate.log 2>&1 &
```
