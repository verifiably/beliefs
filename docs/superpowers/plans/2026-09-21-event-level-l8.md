...
- **`root.py` is the one `atoms` importer** (`test_capability_boundary.py`, `TestTheCompositionRootIsTheOneAtomsImporter`). `world/events.py` imports only `beliefs.errors` and `beliefs.world.logmodel`; `world/verify.py`'s additions import `epoch` and `registry` lazily inside the function, as its neighbours do. The relation calls no write primitive, so `WRITE_ENTRY_POINTS` (`test_permit_boundary.py`) and `CASES` (`test_permit_entry_points.py`) are unchanged — Task 3 asserts `test_permit_boundary.py` and `test_capability_boundary.py` green. (AGENTS.md, Cut plans.)
- **Every discharged cut adds its row to `test_recent_cut_acceptance.py`**: the runner import, the `(runner, cut, accounting)` entry and the guarantee-rows-exercised line (Task 5, not the results task). (AGENTS.md, Cut plans.)
...
| L8-b | L8 | both events first appearing in one cut → `unordered` in both argument orders; no positive answer is emitted |
...
| L8-g | L8 | valid-prefix truncation of A behind E1's captured head: two retained events across chains → `unordered`; the removed event → `EventUnknown` |
...
| L8-j | L8 | same-chain independence: a malformed retained carrier and a malformed world chain leave a same-chain answer standing; the cross-chain question refuses `EpochMalformed` with the carrier alone and answers `unordered` with the world chain alone |
...
| L8-b | `world/verify.py` | `_witnessed` drops the E1-on-B exclusion clause | L8-b |
...
| L8-e | `world/events.py` | `place` ignores the genesis digest (bound to BI-3, below; L8-e's replacement fixture also fails under it only through the head — see BI-3) | BI-3 |
...
Note under the table: the L8-e row's sabotage **is** BI-3's (one site, one arm); the table lists the row so that L8's own coverage reads whole, and the declaration homes the arm on BI-3. That makes **17 arms over 16 units** (L8-a and L8-j home two each; L8-e homes none and is certified by its check alone). "Both directions are required: the check passes on the real tree and fails under sabotage. The runner uses `PREFIX_RUNNERS = ("cut35_acceptance.py",)` and carries `PHASE_MODULES = ("test_event_order_acceptance.py", "test_n2_cut36.py")`."
    ...
    **Refusals are the caller's facts; `unordered` is the evidence's.** An
    ...
    for corpus_id in sorted(carriers):
        with seam.corpus_lock(carriers[corpus_id]):
            views[corpus_id] = seam.inspect_registered(carriers[corpus_id])
    if any(type(view) is not WellFormedView for view in views.values()):
        return "unordered"
    view_a = cast(WellFormedView, views[a.corpus_id])
    view_b = cast(WellFormedView, views[b.corpus_id])
...
**Interfaces:**
- Consumes: `root.event_order`, `root.Event`, `root.log_seam`, `root.init_corpus_root`, `root.init_world_root`, `root.open_corpus`, `root.open_world`, `root.metadata_root_for`, `root.audit_log`, `root.admit_arrival`, `root.replicate_root`, `root.fork_corpus`; `epoch.build_epoch` through `test_world_receipts.publish` and `hold_shipped`; the `work_directory` fixture (`acceptance/conftest.py`); `fixtures_cut4.raw_write`; `test_world_log_audit`'s `chain`, `genesis_entry`, `registration`, `settlement`, `digest`, `ABSENT`, `state`, `Inspections`, `Captures`, `make_seam`.
- Produces: exactly sixteen `test_<unit>_…` functions, one per declaration unit, named as Task 5's `UNIT_CHECKS` lists them.

- [ ] **Step 1: The fixture** — one builder per module, on `test_world_view_acceptance.durable_world`'s shape:
...
`root.log_seam()`; the two stand-in units (L8-i, BI-3) say so in their
docstrings and fabricate only the corpus view they hand the relation.
"""
...
from test_world_log_audit import ABSENT, Captures, Inspections, chain, digest, genesis_entry, make_seam, registration, settlement
from test_world_receipts import hold_shipped, publish

from beliefs import stored
from beliefs.corpus import _operation_lock_for
from beliefs.errors import BuildHold, EpochMalformed, EventCorpusUnknown, EventCorpusUnresolvable, EventUnknown, SubjectMismatch
...
def last_registration(root: Path) -> RegisteredEntryView:
    return [e for e in view_of(root).entries if type(e) is RegisteredEntryView][-1]


def last_intent(root: Path) -> IntentEntryView:
    return [e for e in view_of(root).entries if type(e) is IntentEntryView][-1]


def settlement_of(root: Path, reg: RegisteredEntryView) -> SettledEntryView:
    (found,) = [e for e in view_of(root).entries if type(e) is SettledEntryView and e.registration == reg.digest]
    return found


@pytest.fixture()
def world(work_directory):
    """Two corpora A < B (sorted by id), one world, nothing published yet."""
    roots: list[Path] = []
    ...
    class Built:
        def __init__(self):
            ...
            (self.a, self.alpha, self.writer_a), (self.b, self.beta, self.writer_b) = sorted((first, second), key=lambda c: c[0])
        ...
        def freeze(self):
            """A committed transition in A: the spec freeze L8 names is one
            registered corpus write; its event is the registration."""
            self.writer_a.add(stored.source_node(title="spec", identifiers={"doi": f"10.1/{len(view_of(self.alpha).entries)}"}))
            return Event(self.a, last_registration(self.alpha).digest)

        def intent(self):
            """A run intent in B: every corpus write appends an operation intent
            before its registration; the event is that intent entry."""
            self.writer_b.add(stored.source_node(title="run", identifiers={"doi": f"10.2/{len(view_of(self.beta).entries)}"}))
            return Event(self.b, last_intent(self.beta).digest)
...
- [ ] **Step 2: The units** — sixteen tests, each `_durably`-suffixed as the module's neighbours are:

```python
def test_l8a_a_freeze_before_an_intent_across_ordered_cuts_orders_and_is_antisymmetric_durably(world):
    a = world.freeze()
    world.build()
    b = world.intent()
    world.build()
...
def test_l8b_both_events_first_appearing_in_one_cut_are_unordered_durably(world):
...
def test_l8d_a_cut_covering_one_corpus_establishes_nothing_until_a_covering_pair_does_durably(world):
    a = world.freeze()
    world.build(world.a)  # A only: no anchor for B
    b = world.intent()
    world.build(world.a)
    unordered_both_ways(world, a, b)
    world.freeze()  # a still precedes b: now a covering pair
    world.build()
    world.intent()
    world.build()
    unordered_both_ways(world, a, b)  # b entered before the first covering cut: still unwitnessed
    c = world.freeze()
    world.build()
    d = world.intent()
    world.build()
...
def test_l8e_a_chain_replaced_under_another_fork_genesis_establishes_nothing_durably(world):
    """Cut 9's L4u2 construction: fork A, then present the fork's chain under
    A's subject. Follow `grep -n "L4u2" tests/acceptance/n2_arms_cut9.py tests/acceptance/test_n2_cut9.py`
    for the exact replacement steps (`fork_corpus` into a scratch root, copy its
    `chain/` over A's, keep A's `corpus.yaml`)."""
    a = world.freeze()
    world.build()
    b = world.intent()
    world.build()
    assert world.order(a, b) == "a-precedes-b"
    _replace_chain_with_a_fork(world)  # helper written from the cut-9 fixture
    a2 = world.freeze()
    world.build()
...
def test_l8f_the_double_witness_is_unordered_durably(world):
    """Spec §4.3's schedule, driven one phase at a time through
    `epoch._preflight`, `epoch._capture` and the publication the build performs
    (`grep -n "_preflight\\|_capture\\|_locked_publication_plan" src/beliefs/world/epoch.py`
    for `build_epoch`'s three calls; drive them in that order with the writes
    interleaved as the table says)."""
    e3 = _begin_build(world)            # preflight at h0, capture A
    a = world.freeze()
    e1 = _begin_build(world)            # preflight at h0, capture A then B
    _finish_build(world, e1)            # publish E1
    b = world.intent()
    _capture_b_and_finish(world, e3)    # capture B, publish E3
    world.build()                       # E2, after E1
    world.build()                       # E4, after E3 (and E1)
    unordered_both_ways(world, a, b)


def test_l8g_valid_prefix_truncation_is_unplaceable_for_retained_events_and_unknown_for_removed_ones_durably(world):
    a = world.freeze()
    world.build()
    b = world.intent()
    world.build()
    later = world.freeze()  # beyond E2's captured A head after the next build
    world.build()
    _truncate_chain_to_valid_prefix(world.alpha, behind=later.digest)  # the cut-8 L3 truncation helper
    assert type(view_of(world.alpha)) is WellFormedView
    unordered_both_ways(world, a, b)  # E3's A head is gone, and E1/E2 still witness — assert what §4.3 says:
    with pytest.raises(EventUnknown):
        world.order(later, b)
...
def test_l8h_same_chain_orders_by_ancestry_and_equal_moments_are_unordered_durably(world):
    ...
    reg = last_registration(world.alpha)
    settled = Event(world.a, settlement_of(world.alpha, reg).digest)
...
def test_l8i_a_pending_or_rolled_back_registration_has_no_moment_durably(world):
    """Stand-in inspection: real epochs, a fabricated view for A's chain that
    carries a pending and a rolled-back registration (the engine's own rollback
    needs a halted backend; cut 8 fabricated these by entry class too)."""
    world.freeze()
    world.build()
    live = view_of(world.alpha)
    pending = registration(digest("cut36-pending"), "tx-p", (("p.md", ABSENT),), (("p.md", state_of("p.md")),))
    rolled = registration(digest("cut36-rolled"), "tx-r", (("r.md", ABSENT),), (("r.md", state_of("r.md")),))
    ...
    seam = _seam_with_alpha_view(world, fabricated)
    anchor_event = Event(world.a, live.entries[1].digest)
    for digest_ in (pending.digest, rolled.digest, rolled_back.digest):
        assert verify._event_order(world.config, anchor_event, Event(world.a, digest_), seam=seam) == "unordered"


def test_l8j_same_chain_independence_from_a_malformed_carrier_and_world_chain_durably(world):
    first = world.freeze()
    second = world.freeze()
    ...
    with pytest.raises(EpochMalformed):
        world.order(first, world.intent())
    (world.config.world_root / "epochs" / identity).rename(world.config.world_root / "epochs" / f"{identity}.aside")
    _rewrite_an_interior_world_entry(world.config.world_root)  # the cut-8 L3 interior-damage helper
    assert world.order(first, second) == "a-precedes-b"
    assert world.order(first, world.intent()) == "unordered"
...
def test_l8k_the_refusals_and_a_terminal_corpus_durably(world, work_directory):
    ...
    with pytest.raises(EventUnknown):
        world.order(Event(world.a, world.intent().digest), second)
    twin = Path(mkdtemp(prefix="cut36-twin-", dir=work_directory))
    shutil.copytree(world.alpha, twin, dirs_exist_ok=True)
    with pytest.raises(EventCorpusUnresolvable):
    ...
    lock = _operation_lock_for(world.alpha)
    with lock.capture():
        with pytest.raises(BuildHold):
...
def test_l4a_a_deleted_chain_refutes_against_its_registry_anchor_bound_by_corpus_id_durably(world):
    """Relabel unit: the seven cut-8 and two cut-9 clauses are cited in the cut
    document; this one clause runs durably. Anchor A through the registry
    (`anchor_heads`), delete A's chain, audit selecting A → refuted, the finding
    bound to A's `corpus_id`, B untouched."""
    ...  # follow test_world_log_audit.py's real_audit / exported-head tests for the anchor act and audit call


def test_l10a_a_replica_under_a_fresh_manifest_refuses_subject_mismatch_at_arrival_durably(world, work_directory):
    copy = replicate_root(...)  # cut 8's L10u1 construction: replicate A, rewrite corpus.yaml to a fresh id
    with pytest.raises(SubjectMismatch):
        admit_arrival(world.world, copy, ReplicaOf(world.a), verify.ObserverSet(()), actor="alice")
...
def test_bi1_recovery_precedes_resolution_on_both_paths_durably(world, monkeypatch):
    ...
    order: list[str] = []
    production = log_seam()
    seam = replace(production, inspect_registered=lambda root: order.append(f"inspect:{root == world.config.world_root}") or production.inspect_registered(root))
    original = registry._scan_registry
    monkeypatch.setattr(registry, "_scan_registry", lambda root: order.append("scan") or original(root))
    verify._event_order(world.config, first, second, seam=seam)
    assert order[:2] == ["inspect:True", "scan"]
    order.clear()
    verify._event_order(world.config, first, world.intent(), seam=seam)
    assert order[:2] == ["inspect:True", "scan"]
...
def test_bi2_one_world_inspection_and_the_world_lock_released_before_sorted_unnested_corpus_locks_durably(world):
    ...
    def inspect_registered(root: Path):
        seen.append(root)
        if root != world.config.world_root:
    ...
    assert seen[1:] == [world.alpha, world.beta]


def test_bi3_the_genesis_clause_alone_decides_a_placement_durably(world):
    """Spec §8.2 case 4, isolated: a genuine well-formed view, an anchor with a
    reachable head, only the declared genesis replaced."""
    world.freeze()
    identity = world.build()
    live = view_of(world.alpha)
    with registry._world_lock_for(world.config.world_root):
        (anchor,) = [x for x in epoch._locked_open_epoch(world.config.world_root, identity).anchors if x.subject == world.a]
...
The helpers the units name — `_replace_chain_with_a_fork`, `_begin_build`, `_finish_build`, `_capture_b_and_finish`, `_truncate_chain_to_valid_prefix`, `_rewrite_an_interior_world_entry`, `_seam_with_alpha_view`, `state_of`, and the bodies of L4-a and L10-a — are written in this task from the cut-8 and cut-9 fixtures they cite (`test_n2_cut8.py`'s L3 truncation and interior-damage constructions, `test_n2_cut9.py`'s L4u2 fork replacement and cut 8's L10u1 arrival construction, `test_world_log_audit.py`'s `real_audit` for the anchor-and-audit act). `_seam_with_alpha_view(world, view)` is `replace(log_seam(), inspect_registered=lambda root: view if root == world.alpha else log_seam().inspect_registered(root))`. The double-witness helpers drive `epoch._preflight(world, coverage=…, bindings=…)`, `epoch._capture(world, preflight)` per corpus (split the loop: call `_capture` with a one-corpus coverage twice, or inline its body for A then B with the writes between — the second is what the schedule needs, since `_capture` captures the whole coverage in one call) and the publication step `build_epoch` performs after `_capture` (read `build_epoch`'s body and call the same functions with the draft).
...
| L8-b | `world/verify.py` | `        if not (contains(on_first, first.moment) and excludes(on_second, second.moment)):` | `        if not contains(on_first, first.moment):` |
...
- [ ] **Step 4: The recent-cut row** — `python/tests/test_recent_cut_acceptance.py`: `import cut36_acceptance as cut36`; add `(cut36, 36, (17, 16, 3))` and id `"cut36"` to the parametrization; add
...
```
Read the log when it exits; expected tail: three `[cut36 phase n/3]` lines, `declared arms: 17 (= 16 declaration units; 3 guarantee rows)`, the rows-exercised line, exit 0. Every arm `sound`; every check `resolved`. A `stale` verdict means a `before` no longer matches — fix the declaration (Step 1), never the source.
...
**Placeholders.** Task 4's helper bodies are written from named existing fixtures rather than reproduced here (`test_n2_cut8.py`, `test_n2_cut9.py`, `test_world_log_audit.py`), each named with the `grep` that finds it; the L8-f schedule drives `epoch`'s own three phases. Task 5's `before` strings are the sites Task 3's code defines, copied from the tree at declaration and count-checked.
