...
- Modify: `python/src/science/world/verify.py` (create the module with the seam types), `python/src/science/root.py` (callbacks, view conversion, seam construction), `python/src/science/corpus.py` (`_operation_lock_for`), `python/src/science/world/registry.py` (the lock-only world lookup over `_WORLD_STATES`)
...
**Files:**
- Modify: `python/src/science/world/anchors.py` (act cores), `python/src/science/root.py` (public wrappers `anchor_heads`, `export_head_artifact`), `python/src/science/world/epoch.py` (publication plan gains build-origin log-head `CreateOp`s), `python/src/science/world/errors.py` (`AnchorSubjectUnknown`, `AnchorTargetUnresolvable`)
...
**Files:**
- Modify: `python/src/science/world/verify.py`, `python/src/science/world/errors.py` (`StoreSubjectUnsupported`, `ObserverCarrierInvalid`)
...
- [ ] **Step 1:** Write failing tests: audit over a corpus with damaged node bytes (lock obtainable, verdict returned); audit of a world whose configured id disagrees with genesis (callable; subject-mismatch finding in the report); an unconfigured target root refused; `epochs_ordered` over two sequentially built epochs → ordered; over a fabricated rolled-back publication → unordered, asserted by entry class (cut §6's freeze obligation); the no-sequence-numbers architecture assertion.
...
**Files:**
- Modify: `python/src/science/world/registry.py` (`World.admit` refuses `ReplicaOf`; the shared admission core split), `python/src/science/root.py` (`open_world` at root.py:636 gains the genesis/configuration/mirror agreement check; public wrapper `admit_arrival`), `python/src/science/world/verify.py` (`_admit_arrival` core), `python/src/science/world/errors.py` (`ReplicaAdmissionRequiresVerification`, `ArrivalRefused`, `SubjectMismatch`)
...
- [ ] **Step 4: Collect the evidence, now that the tree is final.** Each under `set -o pipefail`: the full suite (`uv run --frozen pytest` — paste the actual `N passed` summary line), `tools/cut8_acceptance.py` (39 / 23 / 42 / 53, exit 0), the corpus guard's summary line, and `tools/check_guide.py` recorded as **exit 0** (it emits no `N passed` line). Paste all four into the results record's evidence block — this is cut-8 §5's obligation 7, homed here.
