...
- Ledger: `run-confinement` closes at banking; a new finding records that
  **verification publication** crosses the persistence seam and no boundary
  names it. Closed 2026-09-06 by the verification-publication design. The
  roadmap is re-ranked at cut 13 with `workflow-surface` as row 1, and
  Appendix A regenerated by `roadmap_status.py`.
