...
> **Amended 2026-09-24 (publish act, local, cut 40 — `../superpowers/specs/2026-09-23-publish-act-local-design.md` §7; `../plans/2026-09-24-conformance-cut-40-results.md`):** the `publish` kind gains four entry kinds, each in the stored `{kind, subject, outcome}` form with the binding address as `subject`: `publication-request` (`request-corrupt`, with `reason` one of `undecodable`, `intent-disagrees`, `snapshot-missing`, `snapshot-mismatch`, `snapshot-undecodable`); `publication-staging` (`staged` with `corpus_id` and a positive `records` count, or `staging-corrupt` with `corpus_id`, `reason` one of `pins-foreign`, `hole`, `extra`, `bytes`, `marker`, and `refs`, at most one record id); `publication-export` (`exported` with `corpus_id` and `artifact`, or `export-collision` with `corpus_id` and `sibling`, the corpus's own sibling name); and `publication-reveal` (`revealed` with `corpus_id`, or `reveal-refused` with `corpus_id` and `verdict` one of `refuted`, `unresolvable`, `malformed`). A `publish` report's entries form one **ordered sequence**, checked by one shared rule (`report.publish_sequence_error`) at construction and when the next publish's fold reads it: a `request-corrupt` entry alone; or staging, export, reveal in that order, stopping at the first refusal, every earlier entry succeeding; or the whole successful lifecycle followed by the `publication-binding` entry; or, as cut 39's door writes when called bare, the binding entry alone. The pre-binding refusals carry no orphan fields, and the orphan fold skips them.
...
> boundary and closes through exactly one terminal record — the `run` where
> one is minted, the act-report otherwise; T2 closes.
