...
- **World read view and cross-corpus traversal** — an explicit-epoch, map-bound
  ...
  awaits a conformance selection.
- **Coreference attestation and balance** — the governed attributed record,
...
- **Retraction standing at the read** — the evaluator derives and checks the
  ...
  `contract-cut`. A third retraction target arm, the semantic snapshot, is
  read **live** from the corpora its own coverage names and reported or
  refused at import, audit, diagnostic query and the world read; coverage
  narrowing is snapshot succession plus retraction. C8 and C9 close at
...
- **URL retrieval and the acquisition operation** — the `url` locator under
  ...
  lane is closed.
- **The event-level relation across captured corpus heads** — an event is
  `(corpus_id, entry_digest)`; its moment is its own position for a
  genesis, an intent or a committed settlement and its committed
  settlement's position for a registration, a pending or rolled-back
  registration having none; a cut speaks about a chain only with an anchor
  whose genesis is the live genesis and whose head places, both witness
  cuts covering both corpora; the witness predicate over two ordered cuts
  and two chains, ordered cuts decided by one pure helper over one captured
  world view; and the witness-asymmetric relation `root.event_order`
  answering `a-precedes-b`, `b-precedes-a` or `unordered` — the double
  witness two overlapping builds produce answers `unordered`, the log
  design §7's build-window residual made visible — with a same-chain
  question ordered by ancestry and opening no epoch, the refusals
  `EventCorpusUnknown`, `EventCorpusUnresolvable` and `EventUnknown`, and a
  terminal corpus still queryable. L8 closes at cut 36; L4 and L10 close
  there as relabels citing every clause cuts 8, 9 and 10 read; L1 stays
  partial on its persistence arms, re-homed to `persistence-cut`. Three
...
- The **L13 preimage resolver**, built 2026-09-21 at cut 37
  ...
  copy or surviving preimage bytes read on the writable source root through
...
`url-retrieval` left this table and the roadmap's boundary index in the
same commit, and the `acquisition` lane closed with no open boundary. T2
stayed partial on the `audit` and `re-check` operation kinds, so
`act-report-remainder` stayed in this table carrying exactly that remainder,
an off-path boundary of its own (`beliefs-86b150`) rather than a
ride-along; T7 stayed partial on its cross-root case, with
`cross-root-publication`. The record before that
...
certification arm remains with `contract-cut`. The previous record
...
grammar, a kind and a relation signature here. The record before that
...
6. **§9 log design** after the world-index packaging design settles the anchor.
   ...
   **Science's half implemented 2026-08-23** as world-index slice 3
   ...
   (`2026-08-29-successor-admission-design.md`). **Event-level L8 built
   2026-09-21** with cut 36
   (`../superpowers/specs/2026-09-21-event-level-l8-design.md`;
   `../plans/2026-09-21-conformance-cut-36-results.md`): L8 closes, L4 and
   L10 close as relabels, and L1's persistence arms are `persistence-cut`'s.
   **L13's preimage resolver built 2026-09-21** at cut 37
   (`../plans/2026-09-21-conformance-cut-37-results.md`): digest-matched
   classification, stated absence and corruption refusal close L13.
   Row 5 stays partial for L1's persistence arms alone (`persistence-cut`).
