...
(the thirteen kernel kinds — ten until 2026-08-08, eleven until 2026-08-10, twelve until 2026-08-11), §4.2.1/§5.1 (the G3 belief-input closure), §4.3
...
`science` owns the thirteen kernel kinds (kernel §4.2; ten until 2026-08-08, eleven until 2026-08-10, twelve until 2026-08-11), the relation signatures that
...
| D6 | Every consulted profile contract enters belief; activated-but-unconsulted ones do not — **through facets, through claim schemas, and through estimand declarations alike** (claim-schema arm added 2026-08-05, formal model ρA6; estimand arm added 2026-09-16, `2026-09-12-estimand-typing-design.md` §5.4, discharged at conformance cut 31) | **the claim-schema arm:** derive belief over an assessment reading a claim at the operator `affects`, bump the contract **declaring** `affects` while touching **no facet**, and assert `belief_input_digest` **moves** — a walk collecting only facet namespaces reaches no operator and would miss this entirely; then derive belief over an assessment reading `biology/gene-axis`, bump the biology contract, and assert `belief_input_digest` **moves**; activate an unrelated domain and bump *it*, and assert the digest is **unchanged**; **the base-contract arm:** bump the **`science` base contract** in a way that reinterprets the **unnamespaced** `empirical-observation` facet (§3.4) and assert the digest **moves** — a consulted-set rule collecting only *domain* namespaces would miss exactly this, at the eligibility hinge; **the unconditional arm:** take a derivation whose closure reads **no base-profile facet at all**, bump the base contract so it reinterprets a **kernel kind or a relation signature** (`assessment`, `dataset`, `assesses`), and assert the digest **still moves** — membership of `science_contract` is unconditional, so no facet-triggered walk may be able to omit it; **the estimand arm** (added 2026-09-16): derive belief over an assessment whose typed estimand binds a measure sort declared in contract `X`, bump `X` touching **no facet, no operator and no claim**, and assert `belief_input_digest` **moves** — an estimand reaches a contract neither the facet walk nor the claim-schema walk does; bump an activated contract the estimand does **not** reach and assert the digest is **unchanged**; type an estimand under a sort pinned by **no** corpus and assert `ContractDisagreement`; **sabotage:** drop the estimand walk from `consulted_contracts` and assert the first arm **fails**; **negative — the defect this closes:** reinterpret a facet, kind, or signature in a successor contract without changing any facet byte or assessment byte and assert the digest **still moves**, so two beliefs can never share one digest |
...
> **Amended 2026-09-16 — D6's third trigger**
> (`2026-09-12-estimand-typing-design.md` §5.4, discharged at conformance cut
> 31, row Q8). A domain contract gains a fourth declaration class, `estimands:`,
> naming per operator the sorts that operator's contrast levels, measured
> quantity, identification term and conditioning members draw on. An assessment
> carrying a typed estimand therefore reaches contracts that neither the facet
> walk nor the 2026-08-05 claim-schema walk reaches: the contract declaring the
> entry, and the contract declaring each of those sorts. The trigger set widens
> for the same reason it widened then — a walk that collected only the earlier
> two would fail **open**, which limitation 2 below names as the hazard — and
> **D6's asymmetry is inherited unchanged**: activated-but-unconsulted stays
> out, and the base contract stays unconditional. The estimand arm is added to
> D6's cell above rather than to a new row; M6 governs the new declaration
> class **unamended**, and an operator's own `schema_projection()` is
> byte-identical before and after an entry is added for it.
...
2. **"Consulted" must be computed, and the computation is load-bearing.** D6's
   ...
   namespaces, **claim-schema references** (widened 2026-08-05 — formal model
   ρA6) **and estimand declarations** (widened 2026-09-16 —
   `2026-09-12-estimand-typing-design.md` §5.4). An under-collecting walk silently omits a contract from the
