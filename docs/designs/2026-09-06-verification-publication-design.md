...
The failing test the roadmap names is written before the freeze, at
`python/tests/test_verification_identity.py`: over a corpus holding a dataset,
a proposition, an assessment-shaped run, its stored assessment and a replay,
`build_verification` → `admission_record` → the stored verification, then the
stored assessment's identity equals the derived one, the gate's own
verification clause (`lifecycle_state`) reads `admitted` for it, and
`check_verification` reports no contradiction. On this tree it fails at the
first assertion: the stored identity digests `run:<address>` and the derived
one the bare address. The plan's first task makes it pass and widens it to
the full `gather` → `admit` path over `publication_node`.
...
is on this branch at `python/tests/test_verification_identity.py` and fails
on this tree at its first assertion, for the stated reason (run 2026-09-06); `test_designs_corpus.py` passes with the `V` table registered; and
