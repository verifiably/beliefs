...
| **W17** | A coordination address has one standing tip, or resolution refuses naming every tip (added 2026-08-31, coordination-and-view-kinds design) | **Genesis and immutability:** mint a coordination record's first revision and assert it names zero predecessors; edit it and assert a **new whole revision** superseding the first, with the predecessor set stored as **adapter-authored** `supersedes` relations — an author-written one refuses. Assert `add`, `revise`, `supersede` and `retract` all **refuse coordination kinds**, and that the family door refuses world kinds — no other door, in either direction; assert the **pre-plan already-minted guard** holds at the new door, so a replayed genesis refuses rather than silently replacing. **The general rule:** under the per-root lock, supersede a predecessor and then attempt a revision naming it; assert `predecessor-not-standing` judged **at commit**. **The sibling state:** mint sibling successors of one tip in **two corpora**, mount both, and assert resolution returns `Refused(divergent-view)` **naming every tip** — then swap the mount order and assert the identical refusal, pinning that no recency, arrival, or iteration order chooses. **Repair:** mint one revision superseding **all** named tips — the multi-predecessor arity's one job — and assert the address resolves again, with every sibling retained immutable and superseded. **The intent-position rule:** judge admission as a **pure function of a constructed chain prefix** ending at an operation intent: a predecessor already superseded at that position refuses; a predecessor superseded **between** intent and commit admits, and the result is **two standing tips** — the sibling state lawfully reached, repaired the same way. Its operational instance (`publish`'s `publication-binding`) is sub-project 5's cut. **Negative — zero tips is corruption:** raw-write a `supersedes` cycle and assert an **audit finding naming the cycle**, that resolution reports no tip, and that **no repair is offered** — no sanctioned door can produce the state. **Negative — a subordinate needs its project:** attempt a `(project identity, local id)` mint under a project identity with no resolvable record, and under one whose record is divergent; assert both refuse, the second naming the project's tips |
| **W18** | Coordination minting is authorized by the pinned coordination contract, and the tier is world-inert (added 2026-08-31, coordination-and-view-kinds design) | **Authorization:** attempt a coordination mint in a corpus whose manifest pins **no** coordination contract; assert refusal — no pin, no mints. Pin version 1 and attempt a kind only a later amendment declares; assert refusal — an earlier version's pin authorizes nothing an amendment added. **Query admission is the malformed/unresolvable split at the mint boundary:** a view revision whose query fails the grammar or names an out-of-vocabulary kind or version **refuses at mint**; a **well-formed** query whose anchor does not resolve in this checkout is **accepted** — well-formedness is a property of the record, resolvability of the checkout and epoch, and evaluation (the `world-read` lane's) is what refuses the latter, never by denoting the empty set. **World-inertness:** mint coordination records, package an epoch, and assert they appear in **no** world-index map — address, producers, retraction, coreference — and that `belief_input_digest` is **byte-unchanged** by their existence; assert no coordination kind declares a belief-bearing edge and that the consulted-contracts computation never names the coordination contract, whose declarations contain no operator to consult. **Negative — the namespace is reserved:** parse a domain contract claiming the `coordination` namespace and assert refusal, where the `science` reservation is already enforced |
