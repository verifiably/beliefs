...
- The commands the recipes run, should you need one on its own: from `python/`, `uv run --frozen pytest`, `uv run --frozen ruff check .`, and `uv run --frozen pyright`; from `ts/`, `npm test`, `npm run typecheck`, and `npm run check`. `npm ci` is installation, not a gate: `just setup` runs it, once after `git worktree add` and again after a dependency change.
