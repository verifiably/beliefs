...
- `just check` passes on `main` as of 2026-09-07 (ruff and pyright were both red until beliefs-97eb6d and beliefs-0e0c9c). Git hooks are still not installed: `.githooks/` and `git config core.hooksPath .githooks` are the remaining step, and the open question is `hook-pre-push`, which runs the full suite — about 18 minutes here.
